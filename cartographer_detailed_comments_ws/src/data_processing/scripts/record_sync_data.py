...
def callback(imu_msg, velodyne_msg, camera_msg, pose_msg):
    timestamp = imu_msg.header.stamp
    print("Received synchronized messages from IMU, Velodyne, Camera and Pose.")
    ...
    bag.write('/robot_pose', pose_msg, timestamp)
...
if __name__ == '__main__':
    ...
    bag_file = 'bag_file/' + id + '/' + id + '.bag' # ./bag_file/{id}/{id}.bag
    ...
    sync = ApproximateTimeSynchronizer([imu_sub, velodyne_sub, camera_sub, pose_sub], queue_size=50, slop=0.1)
